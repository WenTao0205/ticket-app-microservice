  ...
  <div>
    ...
    <div class="board">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>音乐会热门管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>

    <el-table :data="showList" style="width: 100%" border stripe>
      <el-table-column label="音乐会" header-align="center" label-class-name="tableTitle">
        ...
        <el-table-column label="操作" width="70">
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="添加到音乐会热门" placement="top" :enterable="false" :open-delay="500">
    ...
    <el-table :data="hotShowList" style="width: 100%" border stripe>
      <el-table-column label="音乐会热门" header-align="center" label-class-name="tableTitle">
        <el-table-column prop="title" label="演出名称"></el-table-column>
        <el-table-column prop="hall.name" label="演出场馆" width="300"></el-table-column>
        <el-table-column prop="price" label="演出票价" width="120"></el-table-column>
        <el-table-column prop="startTime" label="开始时间" width="180"></el-table-column>
        ...
        <el-table-column label="操作" width="70">
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="从音乐会热门删除" placement="top" :enterable="false" :open-delay="500">
...
export default {
  ...
  methods: {
    async setShowList() {
      const { data } = await getShowList({ type: '音乐会', rank: '无' })
      this.showList = data
      console.log(this.showList)
    },
    async setHotShowList() {
      const { data } = await getShowList({ type: '音乐会', rank: '音乐会热门' })
    ...
    async addToHot(row) {
      ...
      if(res.status !== 200) {
        this.$message.error('添加音乐会热门失败！')
        return
      }
      await this.setShowList()
      await this.setHotShowList()
      this.$message.success('添加音乐会热门成功！')
    },
    async deleteHot(row) {
      row.hallId = row.hall.id
      row.rank = '无'
      ...
      const res = await updateShow(row)
      if(res.status !== 200) {
        this.$message.error('删除音乐会热门失败！')
        return
      }
      await this.setHotShowList()
      await this.setShowList()
      this.$message.warning('删除音乐会热门成功！')
    }
  }
}
</script>
